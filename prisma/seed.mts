...
async function main() {
  ...
  const usersData = [
    ...
    { id: "u-admin", email: "admin@campus.com", nickname: "운영자", avatar: "⚙️", schoolId: hongik.id, points: 0, level: 1, streak: 0, role: "admin" as const },
  ];
  ...
  // === QUESTIONS & ANSWERS ===
  await prisma.question.upsert({ where: { id: "q1" }, update: {}, create: { id: "q1", courseId: cs101.id, sessionId: cs1.id, authorId: "u-alex", title: "Big-O에서 상수를 무시하는 이유가 뭔가요?", body: "O(2n)이나 O(3n)이나 다 O(n)이라고 하는데, 실제로 2배 3배 차이나는 거 아닌가요?", tags: JSON.stringify(["Big-O", "시간복잡도"]), status: "solved", acceptedAnswerId: "a1", createdAt: ago(9, 5) } });
  await prisma.answer.upsert({ where: { id: "a1" }, update: {}, create: { id: "a1", questionId: "q1", authorId: "u-minji", body: "Big-O는 '증가율'을 나타내는 거라 상수는 의미가 없어요. n이 100만이 되면 O(n)과 O(n²)의 차이가 상수 차이보다 압도적으로 커지거든요.", isAccepted: true, createdAt: ago(9, 4) } });
  ...
  await prisma.question.upsert({ where: { id: "q2" }, update: {}, create: { id: "q2", courseId: cs101.id, sessionId: cs2.id, authorId: "u-jiwoo", title: "Merge Sort와 Quick Sort 중 어떤 걸 써야 하나요?", body: "둘 다 O(n log n)인데 언제 어떤 걸 쓰는 게 좋은지 궁금합니다.", tags: JSON.stringify(["정렬", "Merge Sort", "Quick Sort"]), status: "solved", acceptedAnswerId: "a2", createdAt: ago(6, 5) } });
  await prisma.answer.upsert({ where: { id: "a2" }, update: {}, create: { id: "a2", questionId: "q2", authorId: "u-alex", body: "Quick Sort가 평균적으로 더 빨라요. 하지만 최악의 경우 O(n²)가 될 수 있고, Merge Sort는 항상 O(n log n)이 보장됩니다.", isAccepted: true, createdAt: ago(6, 4) } });
  await prisma.answer.upsert({ where: { id: "a2b" }, update: {}, create: { id: "a2b", questionId: "q2", authorId: "u-hyun", body: "파이썬의 sorted()는 Timsort로 Merge Sort + Insertion Sort 하이브리드예요.", isAccepted: false, createdAt: ago(6, 3) } });

  await prisma.question.upsert({ where: { id: "q3" }, update: {}, create: { id: "q3", courseId: cs101.id, sessionId: cs3.id, authorId: "u-sophia", title: "Stack으로 괄호 매칭하는 코드 예시 있나요?", body: "개념은 이해했는데 실제 코드로 어떻게 구현하는지 감이 안 잡혀요.", tags: JSON.stringify(["Stack", "구현"]), status: "open", createdAt: ago(2, 8) } });
  await prisma.answer.upsert({ where: { id: "a3" }, update: {}, create: { id: "a3", questionId: "q3", authorId: "u-minji", body: "여는 괄호는 push, 닫는 괄호를 만나면 top과 비교 후 pop하면 됩니다!", isAccepted: false, createdAt: ago(2, 6) } });
  ...
  await prisma.answer.upsert({ where: { id: "a4" }, update: {}, create: { id: "a4", questionId: "q4", authorId: "u-alex", body: "1) Leaf 노드: 그냥 삭제\n2) 자식 1개: 부모와 자식을 직접 연결\n3) 자식 2개: in-order successor로 대체", isAccepted: false, createdAt: ago(0, 3) } });

  await prisma.question.upsert({ where: { id: "q5" }, update: {}, create: { id: "q5", courseId: design.id, sessionId: dx1.id, authorId: "u-jiwoo", title: "UX 포트폴리오에 꼭 들어가야 할 요소가 뭔가요?", body: "취업 준비하면서 포트폴리오를 만들고 있는데, 어떤 프로젝트를 넣어야 할까요?", tags: JSON.stringify(["UX", "포트폴리오", "취업"]), status: "open", createdAt: ago(7, 10) } });
  await prisma.answer.upsert({ where: { id: "a5" }, update: {}, create: { id: "a5", questionId: "q5", authorId: "u-emma", body: "핵심은 '과정'을 보여주는 거예요! 문제 정의 → 리서치 → 아이디어 → 프로토타입 → 테스트 결과", isAccepted: false, createdAt: ago(7, 8) } });
  ...
  await prisma.answer.upsert({ where: { id: "a6" }, update: {}, create: { id: "a6", questionId: "q6", authorId: "u-alex", body: "투자자들은 높은 수익률을 쫓아요. 미국 금리가 높으면 달러 자산이 매력적 → 달러 수요 증가 → 달러 강세", isAccepted: true, createdAt: ago(4, 4) } });
  ...
  const events = [
    ...
    { u: "u-alex", e: "answer_accepted", m: { answerer: "민지" }, t: ago(9, 3) },
    ...
    { u: "u-minji", e: "badge_earned", m: { badge: "기여자" }, t: ago(2) },
  ...
  console.log("12 users, 3 courses, 44 notes, 4 summaries, 6 questions, 11 answers, 18 reactions");
  console.log("Demo: minji@hongik.ac.kr / alex@berkeley.edu (pw: password123)");
  console.log("Group: CAMPUS2026 | Courses: CS101JOIN, UXJOIN26, ECON2026");
}
